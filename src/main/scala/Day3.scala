...
object Day3 extends Day(3):
    override def solve(): Unit = 
        ...
        val inverse = partNumbers.map((num, symbols) => symbols.map(s => s -> num)).flatten
        val gears = inverse.map(_._1).distinct.filter(symbol => symbol._1 == '*' && inverse.filter(_._1 == symbol).size == 2)
